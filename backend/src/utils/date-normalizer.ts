...
function parseAtomicDate(rawValue: unknown, boundary: DateBoundary): Date | null {
    ...
    const shortDate = normalizedRaw.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})$/);
    if (shortDate) {
    ...
    if (monthDayYear) {
        ...
        return toSafeUtcDate(Number(monthDayYear[3]), month, Number(monthDayYear[2]));
    }
    ...
    if (dayMonthYear) {
        ...
        return toSafeUtcDate(Number(dayMonthYear[3]), month, Number(dayMonthYear[1]));
    }

    // Month Year
    const monthYearText = normalizedRaw.match(/^([A-Za-z\u00c0-\u017f]+)\s+(\d{4})$/i);
    if (monthYearText) {
...
export function normalizeFlexibleDate(value: unknown, boundary: DateBoundary = 'start'): Date | null {
