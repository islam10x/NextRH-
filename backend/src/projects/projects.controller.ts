...
export class ProjectsController {
    ...
    @Roles(UserRole.TEAM_MANAGER)
    async requestCrossTeamMember(@Body() dto: RequestCrossTeamMemberDto, @Req() req: any) {
        const managerId = req.user?.userId || req.user?.user_id || req.user?.id;
    ...
    async respondCrossTeamRequest(
        ...
        const managerId = req.user?.userId || req.user?.user_id || req.user?.id;
        return this.projectsService.respondCrossTeamRequest(requestId, managerId, dto);
    }
