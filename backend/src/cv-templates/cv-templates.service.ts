...
export class CvTemplatesService {
  ...
  async uploadTemplate(
    ...
    const { relativePath, filename } =
      await this.fileStorageService.saveTemplateFile(file, dto.templateName);
  ...
  async replicateTemplate(
    ...
    const { relativePath, filename } =
      await this.fileStorageService.replicateTemplateFile(
        template.filePath,
        name,
      );
  ...
  async getTemplateFile(templateId: string) {
    ...
    const absPath = this.fileStorageService.resolveFromWorkspace(
      template.filePath,
    );
  ...
  async getOwnedTemplate(
    ...
    if (!template.uploadedBy || template.uploadedBy.user_id !== userId) {
      throw new ForbiddenException(
        "You can only use templates from your own history",
      );
    }
    return template;
  }
  ...
  async recordGenerationUsage(args: {
    ...
    try {
      ...
      saved = await this.fileStorageService.saveTemplateFile(
        file,
        friendlyName,
      );
    } catch (exc) {
  ...
  private normalizeLanguageForStorage(
  ...
  private assertTemplateFile(file?: Express.Multer.File) {
    ...
    if (!allowed) {
      throw new BadRequestException(
        "Only .docx or .pdf templates are supported",
      );
    }
  }
}
