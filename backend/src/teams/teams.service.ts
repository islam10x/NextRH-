...
export class TeamsService {
  ...
  private async findOrCreateManagerTeam(
  ...
  async getManagerTeam(managerUserId: string) {
    ...
    if (!manager || manager.role !== UserRole.TEAM_MANAGER) {
      throw new BadRequestException(
        "Only team managers can manage team settings",
      );
    }
  ...
  async getTeamInfoForEmployee(memberUserId: string) {
    ...
    return {
      ...
      managerName: member.team.manager
        ? `${member.team.manager.firstName || ""} ${member.team.manager.lastName || ""}`.trim() ||
          member.team.manager.email
        : null,
    };
  }
  ...
  async listAllWithMembers(): Promise<
    { teamId: string; teamName: string; memberUserIds: string[] }[]
  > {
  ...
  async getTeamNamesForManagerEmails(
    ...
    const teamNamesByManagerId = await this.getTeamNamesForManagers(
      managers.map((m) => m.user_id),
    );
    for (const manager of managers) {
      result.set(
        manager.email,
        teamNamesByManagerId.get(manager.user_id) ||
          this.defaultTeamNameForManager(manager),
      );
    }
