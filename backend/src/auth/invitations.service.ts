...
export class InvitationsService {
  ...
  async inviteEmployee(email: string, role: string, invitedByUserId: string) {
    ...
    if (existingUser) {
      if (
        ...
        existingUser.status === UserStatus.PENDING_INVITATION
      ) {
        if (isTeamManager) {
          ...
          if (existingUser.status === UserStatus.ACTIVE) {
            await this.mailService.sendTeamAddedEmail(
              existingUser.email,
              managerName,
              manager?.email,
            );
          ...
          message:
            existingUser.status === UserStatus.ACTIVE
              ...
              : isTeamManager
                ...
                : "User is already pending registration",
        };
      }
      throw new BadRequestException(
        "User account is disabled or in an invalid state",
      );
    }
  ...
  async resendInvitation(userId: string, invitedByUserId: string) {
    ...
    if (user.status !== UserStatus.PENDING_INVITATION) {
      throw new BadRequestException(
        "Can only resend invitations for pending users",
      );
    }

    // 1. Invalidate old tokens for this user
    await this.invitationTokenRepository.update(
      { userId, isUsed: false },
      { isUsed: true },
    );
