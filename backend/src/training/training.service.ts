...
export class TrainingService {
  ...
  async listForEmployee(profileId: string) {
    ...
    return rows.map((row) => ({
      ...
      assignedByName: row.assignedBy
        ? managerNameByEmail.get(row.assignedBy.toLowerCase()) || "Manager"
        : null,
    }));
  }
  ...
  async updateStatus(
    ...
    if (dto.status === "completed" && training.profile?.profile_id) {
      try {
        ...
        await this.scoringService.computeScore(
          training.profile.profile_id,
          year,
        );
        this.logger.log(
          `Auto-recomputed score for profile ${training.profile.profile_id} after training completion`,
        );
      } catch (err: any) {
        this.logger.warn(
          `Failed to auto-recompute score after training completion: ${err.message}`,
        );
      }
    }
  ...
  async uploadProof(
    ...
    const manualDate = issueDate
      ? this.toDateOrThrow(issueDate, "Invalid issue date")
      : null;
    ...
    if (effectiveIssueDate && training.dueDate) {
      ...
      if (
        !isNaN(due.getTime()) &&
        effectiveIssueDate.getTime() > due.getTime()
      ) {
        throw new BadRequestException(
          "Issue date must be on or before the due date",
        );
      }
    }
    ...
    const issuerUsed =
      parsedIssuer && parsedIssuer.trim()
        ? parsedIssuer.trim()
    ...
    const issueDateIso = effectiveIssueDate
      ? effectiveIssueDate.toISOString().slice(0, 10)
      : undefined;
    ...
    const metadataPayload = {
      ...
      expiration: cert.expirationDate
        ? cert.expirationDate.toISOString().slice(0, 10)
        : undefined,
    };

    await this.fileStorageService.addCertificationToMetadata(
      userId,
      metadataPayload,
    );
    ...
    if (training.profile?.profile_id) {
      try {
        ...
        this.logger.log(
          `Auto-recomputed score for profile ${training.profile.profile_id} after proof upload`,
        );
      } catch (err: any) {
        this.logger.warn(
          `Failed to auto-recompute score after proof upload: ${err.message}`,
        );
      }
    }
  ...
  private async extractCertMetadataFromFile(
    ...
    const maxAttempts =
      Number.isFinite(maxAttemptsFromEnv) && maxAttemptsFromEnv > 0
        ? Math.floor(maxAttemptsFromEnv)
        : 3;
    ...
    if (!resp) {
      this.logger.error(
        `AI service unreachable after retries: ${lastNetworkErrorDetails || "unknown error"}`,
      );
    ...
    if (!data || typeof data !== "object") {
      this.logger.error(
        `AI service returned non-JSON response: ${String(data)}`,
      );
      throw new BadRequestException(
        "AI parsing service returned an invalid response. Please retry.",
      );
    }
    ...
    if (dateStr) {
      ...
      if (!isNaN(parsed.getTime())) {
        ...
        this.logger.warn(
          `AI returned an unparseable date string: "${dateStr}"`,
        );
      }
    } else {
      this.logger.warn(
        "AI response contained no date field — will use manual date or null",
      );
    }
    ...
    const issuer: string | null = data.issuer || data?.metadata?.issuer || null;
  ...
  private assertEmployeeNameMatchOrThrow(
    rawText: string | null,
    firstName?: string,
    lastName?: string,
  ) {
    ...
    if (!rawText || !rawText.trim()) {
      throw new BadRequestException(
        "Unable to verify certificate owner name from parsed document.",
      );
    }
    ...
    if (
      ...
      normalizedText.includes(reversed)
    ) {
      return;
    }
    ...
    this.logger.warn(
      `Name match failed for profile name "${first} ${last}", bestScore=${best.toFixed(3)}`,
    );
    throw new BadRequestException(
      "Certificate owner name does not match your profile name.",
    );
  }
