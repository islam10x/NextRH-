...
export class FileStorageService {
  ...
  async saveEmployeeFile(
    ...
    if (existingBaseDir) {
      ...
      const expectedFolderName = this.buildSafeFolderName(
        employeeName,
        user.user_id,
      );
      ...
      if (currentFolderName !== expectedFolderName) {
        ...
        this.logger.log(
          `[Folder Rename] Renaming from ${currentFolderName} to ${expectedFolderName}`,
        );
      ...
      baseDir = await this.resolveEmployeeBaseDir(
        employeeName,
        user.user_id,
        user.email,
      );
    ...
    this.logger.log(
      `[CV Upload] User: ${userId}, Employee Name: ${employeeName}, Base Dir: ${baseDir}`,
    );
      ...
      const certificationNames = rawCertifications
        .map((cert: any) => {
          ...
          if (cert && typeof cert === "object")
            return cert.name || cert.certification_name || "";
          return "";
        })
  ...
  private async resolveEmployeeBaseDir(
    employeeName: string,
    userId: string,
    email: string,
  ) {
    ...
    this.logger.log(
      `[Folder Creation] Root: ${rootDir}, Folder Name: ${folderName}, Full Path: ${baseDir}`,
    );
  ...
  private async resolveStoredName(
    ...
    if (preferredRaw) {
      ...
      const preferredExt =
        preferredParsed.ext || this.resolveExtension(file, safeOriginalName);
  ...
  private async removeConflictingFiles(targetDir: string, baseName: string) {
    try {
      ...
      this.logger.warn(
        `Failed to remove conflicting files in ${targetDir}: ${error.message}`,
      );
    }
  }
  ...
  async saveMetadata(userId: string, parsedData: any) {
    ...
    if (!baseDir) {
      this.logger.warn(
        `No base directory found for user ${userId}, cannot save metadata`,
      );
      return;
    }
    ...
    const toCertificationEntry = (
      ...
      const isUploaded =
        forceUploaded ?? Boolean(cert.is_uploaded ?? cert.isUploaded);
      const issuer = String(
        cert.issuer ||
      ...
      if (dateObtained && dateObtained !== issueDate)
        entry.date_obtained = dateObtained;
    ...
    const existingCertificationsRaw = Array.isArray(
    ...
    const verifiedCertifications = existingCertifications.filter(
      (cert) => cert.is_uploaded,
    );
    ...
    const enhancedMetadata = {
      ...
      experience_years:
        typeof existingMetadata.experience_years === "number"
          ? existingMetadata.experience_years
          : 0,
  ...
  async updateExperienceYearsInMetadata(
    ...
    if (!baseDir) {
      this.logger.warn(
        `No base directory found for user ${userId}, cannot update experience years`,
      );
      return;
    }
    ...
    this.logger.log(
      `Updated metadata experience_years to ${metadata.experience_years} for user ${userId}`,
    );
  }
  ...
  async addCertificationToMetadata(
    ...
    if (!baseDir) {
      this.logger.warn(
        `No base directory found for user ${userId}, cannot update metadata`,
      );
      return;
    }
    ...
    try {
      ...
      if (existingIndex >= 0) {
        ...
        metadata.certifications[existingIndex] = {
          ...
          is_uploaded: Boolean(
            current.is_uploaded || certificationPayload.is_uploaded,
          ),
        };
      } else {
      ...
      await fs.writeFile(metadataPath, JSON.stringify(metadata, null, 2));
      this.logger.log(
        `Updated metadata with certification payload: ${JSON.stringify(certificationPayload)}`,
      );
    } catch (error) {
      this.logger.error(
        `Error updating metadata with certification: ${error.message}`,
      );
    }
  }
  ...
  public getWorkspaceRoot(): string {
    ...
    while (currentIdx !== path.parse(currentIdx).root) {
      ...
      if (
        existsSync(path.join(potentialRoot, "backend")) &&
  ...
  async saveTemplateFile(file: Express.Multer.File, preferredName?: string) {
    ...
    const preferredSafe = preferredName
      ? this.sanitizeFileName(preferredName)
      : "";
    ...
    const ext =
      preferredParsed?.ext ||
      originalParsed.ext ||
      this.resolveExtension(file, originalName) ||
      ".docx";
  ...
  async replicateTemplateFile(existingPath: string, preferredName?: string) {
    ...
    const safeBase = this.sanitizeFileName(
      preferredName || parsed.name || "template",
    );
