...
export class MailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!host || !user) {
      console.warn(
        "WARNING: SMTP configuration is missing. Emails will fail to send.",
      );
    }
  ...
  async sendInvitationEmail(
    ...
    const fromDisplayName = senderName
      ? `${senderName} via CV Manager`
      : "CV Manager";
  ...
  async sendTrainingAssignedEmail(params: {
    ...
    const systemFrom =
      this.configService.get<string>("MAIL_FROM_ADDRESS") ||
      this.configService.get<string>("SMTP_FROM");
    const fromDisplayName = params.managerName
      ? `${params.managerName} via CV Manager`
      : "CV Manager";

    const frontend = this.getPrimaryFrontendUrl();
    const ctaUrl =
      params.trainingUrl || frontend + "/employee/training-projects";
    const dueLine = params.dueDate
      ? `<p style="margin: 6px 0;">Due date: <strong>${params.dueDate}</strong></p>`
      : "";
  ...
  async sendTeamAddedEmail(
    ...
    const fromDisplayName = managerName
      ? `${managerName} via CV Manager`
      : "CV Manager";
    const frontend = this.getPrimaryFrontendUrl();
