...
export class CvGenerationService {
    ...
    async generate(dto: GenerateCvDto, generatorUserId: string) {
        const template = await this.templateRepo.findOne({ where: { template_id: dto.templateId } });
        if (!template) {
        ...
        const outputDir = await this.fileStorageService.getGeneratedCvDir(dto.employeeId);
        const templatePath = this.resolveAbsolutePath(template.filePath || '');

        let outputFormats = dto.outputFormats?.length ? dto.outputFormats : ['docx', 'pdf'];
        ...
        const record = this.generatedRepo.create({
            ...
            generatedBy: generatorUserId ? ({ user_id: generatorUserId } as User) : null,
        ...
        try {
            ...
            saved.status = 'failed';
            saved.errorMessage = error instanceof Error ? error.message : String(error);
        ...
        if (aiResponse.field_mapping && !template.fieldMapping) {
            ...
            this.logger.log(`Cached field mapping for template ${template.template_id}`);
        }
        ...
        return {
            ...
            downloadPdfUrl: pdfPath ? `/cv-generation/${saved.generated_cv_id}/download?format=pdf` : null,
        };
    }
    ...
    private async callAiService(payload: Record<string, any>): Promise<AiGenerateResponse> {
