...
export class FileValidationService {
    ...
    private validateSize(file: Express.Multer.File) {
        const sizeBytes = typeof file.size === 'number' ? file.size : file.buffer?.length || 0;
    ...
    private async scanForViruses(file: Express.Multer.File, context: UploadContext) {
        const clamEnabled = this.configService.get<string>('CLAMAV_ENABLED') === 'true';
        if (!clamEnabled) {
            return;
        }
    ...
    private async getClamClient() {
        ...
        const timeout = Number(this.configService.get<string>('CLAMAV_TIMEOUT') || 60000);

        this.clamClientPromise = new NodeClam().init({
            clamdscan: {
                host,
                port,
                timeout,
            ...
            this.logger.error(`Failed to initialize ClamAV client: ${error?.message || error}`);
            return null;
        });
