...
export class CvController {
    ...
    FileInterceptor("template", {
      ...
      fileFilter: (_req, file, cb) => {
        ...
        if (!allowed.includes((file.mimetype || "").toLowerCase())) {
          return cb(
            new BadRequestException("Template must be .docx or .pdf format"),
            false,
          );
        }
        return cb(null, true);
  ...
  async generateFromHistory(
    ...
    if (!uuidRegex.test(body.templateId) || !uuidRegex.test(body.employeeId)) {
      throw new BadRequestException(
        "templateId and employeeId must be valid UUIDs",
      );
    }
  ...
  async generateFromStored(
    ...
    if (!body.templateEmployeeId || !body.targetEmployeeId) {
      throw new BadRequestException(
        "templateEmployeeId and targetEmployeeId are required",
      );
    }
    ...
    const uuidRegex =
      /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    if (
      !uuidRegex.test(body.templateEmployeeId) ||
      !uuidRegex.test(body.targetEmployeeId)
    ) {
      throw new BadRequestException(
        "templateEmployeeId and targetEmployeeId must be valid UUIDs",
      );
    }
