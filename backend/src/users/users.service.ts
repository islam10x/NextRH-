...
export class UsersService {
    ...
    async findById(id: string): Promise<User> {
        const user = await this.usersRepository.findOne({
            where: { user_id: id },
            select: ['user_id', 'email', 'firstName', 'lastName', 'role', 'status', 'createdAt', 'updatedAt', 'avatarPath'],
        });
    ...
    async updateOwnProfile(userId: string, updateMyProfileDto: UpdateMyProfileDto) {
        const user = await this.findById(userId);
    ...
    async updateAvatar(userId: string, file: Express.Multer.File) {
        ...
        const relativePath = path.relative(this.getStorageRoot(), fullPath).replace(/\\/g, '/');
    ...
    private async resolveUserBaseDir(user: User) {
        ...
        if (existsSync(rootDir)) {
            ...
            const existing = folders.find((folder) => folder.endsWith(`_${userIdShort}`));
            if (existing) {
    ...
    private async removeExistingAvatarFiles(uploadDir: string) {
        try {
            ...
            this.logger.warn(`Failed to clear old avatar files: ${error instanceof Error ? error.message : error}`);
        }
    }
}
