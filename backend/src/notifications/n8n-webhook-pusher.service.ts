...
export class N8nWebhookPusher {
  ...
  constructor(
    ...
    this.webhookUrl = this.configService.get<string>("N8N_WEBHOOK_URL") || null;
    this.webhookSecret =
      this.configService.get<string>("N8N_WEBHOOK_SECRET") || "";
    this.isHttps = Boolean(
      this.webhookUrl && this.webhookUrl.startsWith("https://"),
    );
  ...
  push(payload: {
    ...
    this.logger.log(`Pushing notification ${payload.notificationId} to n8n...`);
  ...
  private sendWebhook(
    ...
    firstValueFrom(
      this.httpService.post(this.webhookUrl, payload, {
        ...
        httpAgent: this.isHttps
          ? undefined
          : new http.Agent({ keepAlive: false }),
        httpsAgent: this.isHttps
      ...
      .then((response) => {
        if (response.status >= 200 && response.status < 300) {
          this.logger.log(`n8n push succeeded for ${payload.notificationId}`);
          return;
        }
      ...
      .catch((err) => {
        ...
        if (
          ...
          (err?.code === "ECONNRESET" ||
            ...
            /ECONNRESET/i.test(String(err?.message || "")))
        ) {
          this.logger.warn(
