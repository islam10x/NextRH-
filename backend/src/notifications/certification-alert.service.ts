    ...
    async handleCertificationExpiryCheck() {
        ...
        try {
            ...
            const expiringCerts = await this.certRepo
                ...
                .andWhere('c.expiration_date > :now', { now: now.toISOString().slice(0, 10) })
                .andWhere('c.expiration_date <= :in30', { in30: in30Days.toISOString().slice(0, 10) })
                .getMany();
            ...
            this.logger.log(`Certification expiry check complete. ${created} new notification(s) created.`);
        } catch (error) {
