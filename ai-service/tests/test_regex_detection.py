...
for fname, expected_name_part, expected_phone in CASES:
    ...
    d = _detect_personal_info(full, paras)
    pairs = _build_replacements(d, EMPLOYEE)
    ...
    issues = []
    if not ok_name:  issues.append(f"name '{expected_name_part}' not in {old_vals}")
    if not ok_phone: issues.append(f"phone '{expected_phone}' not in {old_vals}, detected={d.get('phone')}")
    if not ok_email: issues.append(f"email not found, detected={d.get('email')}")
